...
class PackGiverController {
    async execute({ nickname, packId }) {
        ...
        console.log('boolean', Boolean(pack?.toJSON()?.case), 'no boolean', pack?.toJSON()?.case, pack?.toJSON());
        const commandToGive = Boolean(pack?.toJSON()?.case) === true ? settings?.caseGiveCmd : settings?.donGiveCmd;
